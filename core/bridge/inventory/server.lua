    ...
    if GetResourceState("ox_inventory") ~= 'missing' then
        Functions.Inventory = {
            ...
            OpenInventory = function(src, type)
                ...
                local mystash = exports.ox_inventory:CreateTemporaryStash({ label = 'Custom Stash', slots = 50, maxWeight = 500000, items = items })
            ...
            GetInventory = function(src, name)
                ...
                return {items = json.decode(inventory.data)}
            end,
...
elseif GetResourceState("qb-core") ~= 'missing' then
    if GetResourceState("qb-inventory") ~= 'missing' then
        Functions.Inventory = {
            ...
            OpenInventory = function(src, type)
                return exports['qb-inventory']:OpenInventory(src, type, { label = 'Custom Stash', maxweight = 5000000, slots = 50 })
            end,
